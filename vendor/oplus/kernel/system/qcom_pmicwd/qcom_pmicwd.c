...
static int pmicwd_kthread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		schedule_timeout_interruptible(msecs_to_jiffies((((pon->pmicwd_state >> 8) &
					       0xff) * 1000) / 2));
				...
				size_t count, loff_t *off)
{
	...
	len = snprintf(&page[len], 128 - len, "enable = %d timeout = %d rstype = %d\n",
		       pon->pmicwd_state & 0xff, (pon->pmicwd_state >> 8) & 0xff,
		       (pon->pmicwd_state >> 16) & 0xff);
				 ...
				 size_t count, loff_t *off)
{
	...
	if (tmp_rstypt >= PON_POWER_OFF_MAX_TYPE
			|| tmp_rstypt <= PON_POWER_OFF_RESERVED) {
	...
	new_state = (tmp_enable & 0xff) | ((tmp_timeout & 0xff) << 8) | ((
				tmp_rstypt & 0xff) << 16);
...
static int pmicwd_test_set(void *data, u64 val)
{
	...
	if (val) {
		...
		if (pon->wd_task) {
			kthread_stop(pon->wd_task);
			oplus_qpnp_pon_wd_config(
				1);  // wd will be disabled in wd_task stop flow, so re-enabled
			pon->wd_task = NULL;
			dev_info(pon->dev,
				 "%s: pmicwd test start: after %d seconds reboot or enter RAMDUMP! \n", __func__,
				 (pon->pmicwd_state >> 8) & 0xFF);
...
void pmicwd_init(struct platform_device *pdev, struct qpnp_pon *pon,
		 bool sys_reset)
{
	u32 pon_rt_sts = 0;
	int rc;
