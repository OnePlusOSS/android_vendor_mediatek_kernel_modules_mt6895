				       ...
				       size_t cnt, loff_t *data)
{
	...
	if (val > SHUTDOWN_RUS_MIN) {
		...
		temp = val / 256;
		gtotaltimeout = (temp < SHUTDOWN_TOTAL_TIME_MIN) ? SHUTDOWN_TOTAL_TIME_MIN :
				temp;// for safe
		pr_info("shutdown_detect_trigger rus val %ld %d %d %d\n", val, gnativetimeout,
			gjavatimeout, gtotaltimeout);
		return cnt;
	}
	...
	case SHUTDOWN_STAGE_KERNEL:
		...
		if ((shutdown_kernel_start_time - shutdown_init_start_time) > gnativetimeout) {
			...
			if (((shutdown_init_start_time - shutdown_systemserver_start_time) >
					gjavatimeout) && shutdown_systemserver_start_time) {
	...
	case SHUTDOWN_STAGE_INIT:
		if (!shutdown_detect_started) {
			...
			shd_complete_monitor = kthread_run(shutdown_detect_func, NULL,
...
{
	if (0 != dump_kmsg(OPLUS_PARTITION_OPPORESERVE3_LINK,
