			  ...
			  struct binder_transaction_data *tr)
{
	...
	if (!(tr->flags & TF_ONE_WAY) /*report sync binder call*/
		...
		hans_report(SYNC_BINDER, task_tgid_nr(proc->tsk), task_uid(proc->tsk).val,
			    task_tgid_nr(target_proc->tsk), task_uid(target_proc->tsk).val, "SYNC_BINDER",
			    -1);
	...
	if (!(tr->flags & TF_ONE_WAY) /*report sync binder call*/
			...
			&& (NULL != proc->tsk)
			&& (task_uid(target_proc->tsk).val > MIN_USERAPP_UID
			    || task_uid(target_proc->tsk).val == HANS_SYSTEM_UID) //uid >10000
			&& is_belong_cpugrp(target_proc->tsk))
		hans_report(SYNC_BINDER_CPUCTL, task_tgid_nr(proc->tsk),
			    task_uid(proc->tsk).val, task_tgid_nr(target_proc->tsk),
			    task_uid(target_proc->tsk).val, "SYNC_BINDER_CPUCTL", -1);
			  ...
			  struct binder_transaction_data *tr)
{
	...
	if (target_proc
			...
			&& is_frozen_tg(target_proc->tsk))
		hans_report(SYNC_BINDER, task_tgid_nr(proc->tsk), task_uid(proc->tsk).val,
			    task_tgid_nr(target_proc->tsk), task_uid(target_proc->tsk).val,
			    "SYNC_BINDER_REPLY", -1);
}
			 ...
			 struct task_struct *dst)
{
	if (is_frozen_tg(dst)  /*signal receiver thread group is frozen?*/
		...
		if (hans_report(SIGNAL, task_tgid_nr(killer), task_uid(killer).val,
			...
			printk(KERN_ERR
			       "HANS: report signal-freeze failed, sig = %d, caller = %d, target_uid = %d\n",
