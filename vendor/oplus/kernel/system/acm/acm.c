...
static void acm_nomediadir_list_add(struct list_head *head,
				    struct acm_ndnode *node)
{
...
static int inquiry_create_nomedia_policy(struct dentry *dentry)
{
	...
	list_for_each_entry(dir_node, &acm_nomediadir_list.head, lnode) {
		if (strlen(dentry_path) >= strlen(dir_node->afnd.dir)) {
			if (strncasecmp(dentry_path + (strlen(dentry_path) - strlen(
							       dir_node->afnd.dir)),
...
{
	if (strncasecmp(dentry->d_name.name, ACM_NOMEDIA_FNAME,
			strlen(ACM_NOMEDIA_FNAME)) == 0)
