...
int bright_screen_timer_restart(void)
{
	...
	if (g_bright_data.status != BRIGHT_STATUS_CHECK_ENABLE
			&& g_bright_data.status != BRIGHT_STATUS_CHECK_DEBUG) {
	...
	if (g_bright_data.blank == FB_BLANK_UNBLANK) {
		...
		mod_timer(&g_bright_data.timer,
			  jiffies + msecs_to_jiffies(g_bright_data.timeout_ms));
...
static void bright_error_happen_work(struct work_struct *work)
{
	...
	if (bri_data->error_count == 0) {
		...
		sprintf(g_bright_data.error_id, "%d.%ld.%ld", get_systemserver_pid(), ts.tv_sec,
			ts.tv_nsec);
#else
		struct timespec ts;
		getnstimeofday(&ts);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int bright_screen_cancel_proc_show(struct seq_file *seq_file,
		void *data)
{
...
static int bright_screen_cancel_proc_open(struct inode *inode,
		struct file *file)
{
