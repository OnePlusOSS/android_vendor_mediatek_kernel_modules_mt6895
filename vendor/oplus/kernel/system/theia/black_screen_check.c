...
int black_screen_timer_restart(void)
{
	...
	if (g_black_data.blank == FB_BLANK_POWERDOWN) {
		...
		mod_timer(&g_black_data.timer,
			  jiffies + msecs_to_jiffies(g_black_data.timeout_ms));
...
{
	struct black_data *bla_data    = container_of(work, struct black_data,
					 error_happen_work);
	...
	if (bla_data->error_count == 0) {
		...
		sprintf(g_black_data.error_id, "%d.%ld.%ld", get_systemserver_pid(), ts.tv_sec,
			ts.tv_nsec);
#else
		struct timespec ts;
		getnstimeofday(&ts);
