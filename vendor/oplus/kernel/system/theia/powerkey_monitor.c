		...
		size_t count, loff_t *off)
{
	...
	len = sprintf(&page[len],
		      "status=%d timeout_ms=%u is_panic=%d get_log=%d systemserver_pid=%d\n",
...
static int powerkey_monitor_param_proc_show(struct seq_file *seq_file,
		void *data)
{
...
static int powerkey_monitor_param_proc_open(struct inode *inode,
		struct file *file)
{
		...
		size_t count, loff_t *off)
{
	...
	POWER_MONITOR_DEBUG_PRINTK("enter theia_powerkey_report_proc_read %d  %d",
				   count, *off);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int theia_powerkey_report_proc_show(struct seq_file *seq_file,
		void *data)
{
	seq_printf(seq_file, "%s called\n", __func__);
	return 0;
...
static int theia_powerkey_report_proc_open(struct inode *inode,
		struct file *file)
{
		...
		size_t count, loff_t *off)
{
	...
	if (!strcmp(buffer, "test_d_block\n")) {
		...
		*/
		...
		SendTheiaKevent(THEIA_KEVENT_TYPE_COMMON_STRING, "logTagTest", "eventIDTest",
				"Powerkey test kevent!");

	return count;
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int theia_powerkey_test_node_proc_open(struct inode *inode,
		struct file *file)
{
	int ret;
...
int __init powerkey_monitor_init(void)
{
	...
	proc_create("pwkMonitorParam", S_IRWXUGO, NULL,
		    &powerkey_monitor_param_proc_fops);

	/*a node for normal stage record*/
	proc_create("theiaPwkReport", S_IRWXUGO, NULL,
