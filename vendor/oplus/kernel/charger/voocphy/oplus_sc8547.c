...
static int sc8547_set_dpdm_enable(struct oplus_voocphy_manager *chip, bool enable)
{
        if (!chip) {
                pr_err("Failed\n");
                return -1;
        ...
        if (enable)
		return sc8547_write_byte(chip->client, SC8547_REG_21, 0x21);
        else
		return sc8547_write_byte(chip->client, SC8547_REG_21, 0x0);
}
...
irqreturn_t sc8547_quick_handler(int irq, void *dev_id) 
{
	pr_err("quick handler\n");
...
static int sc8547_irq_register(struct oplus_voocphy_manager *chip)
{
	...
	if (chip->irq) {
		ret = request_threaded_irq(chip->irq, sc8547_quick_handler,
...
static struct oplus_voocphy_operations oplus_sc8547_ops = {
	...
	.set_dpdm_enable	= sc8547_set_dpdm_enable,
};
