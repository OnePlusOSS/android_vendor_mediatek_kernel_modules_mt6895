...
#define CHARGER_UP_CPU_FREQ_SET0	2000000
#define CHARGER_UP_CPU_FREQ_SET4	2450000
...
static struct freq_qos_request freq_qos_req0, freq_qos_req4;
#endif
...
int base_cpufreq_for_chg = CHARGER_UP_CPU_FREQ_SET0;
...
{
	struct cpufreq_policy *policy0, *policy4;
	int ret;
	static bool initialized = false;

	policy0 = cpufreq_cpu_get(0);
	policy4 = cpufreq_cpu_get(4);
	if (!policy0 || !policy4) {
		pr_err("%s: cpufreq policy not found for cpu0/4\n", __func__);
		return;
	}

	if (!initialized) {
		ret = freq_qos_add_request(&policy0->constraints, &freq_qos_req0, FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
		ret = freq_qos_add_request(&policy4->constraints, &freq_qos_req4, FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
		if (ret < 0) {
	...
	if (flag == 1) {
		ret = freq_qos_update_request(&freq_qos_req0, CHARGER_UP_CPU_FREQ_SET0);
		if(g_oplus_chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT) {
			ret = freq_qos_update_request(&freq_qos_req4, CHARGER_UP_CPU_FREQ_SET4);
		}
	} else {
		ret = freq_qos_update_request(&freq_qos_req0, FREQ_QOS_MIN_DEFAULT_VALUE);
		if(g_oplus_chip->boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT) {
			ret = freq_qos_update_request(&freq_qos_req4, FREQ_QOS_MIN_DEFAULT_VALUE);
		}
	}
...
{
#ifndef CONFIG_OPLUS_CHARGER_MTK /*debug for 6895*/
	if (!g_voocphy_chip)
		return;
	voocphy_info("%s \n", __func__);
...
int oplus_voocphy_vbus_vbatt_detect(struct oplus_voocphy_manager *chip)
{
	...
	if (oplus_voocphy_get_bidirect_cp_support()) {
		...
		if (chip->vbus < 3700) {
			oplus_tcpc_direct_charge_en(true);
		} else {
...
int oplus_voocphy_curr_event_handle(unsigned long data)
{
	...
	if (chip->voocphy_dual_cp_support) {
		...
		voocphy_err("slave_status = %d, cp_ichg=%d\n",
				slave_cp_enable,
				chip->cp_ichg);
		if ((chip->cp_ichg > chip->slave_cp_enable_thr)
			...
			if (oplus_voocphy_check_slave_cp_status()) {
				voocphy_err("slave cp is normal, then set current! current_max=%d\n", chip->current_max);
			} else {
