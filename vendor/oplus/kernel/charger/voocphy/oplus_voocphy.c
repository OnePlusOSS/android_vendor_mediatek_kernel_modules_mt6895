...
void cpuboost_charge_event(int flag)
{
	...
	if (!initialized) {
		ret = freq_qos_add_request(&policy->constraints, &freq_qos_req, FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
		if (ret < 0) {
	...
	if (flag == 1) {
		...
		ret = freq_qos_update_request(&freq_qos_req, FREQ_QOS_MIN_DEFAULT_VALUE);
	}
