...
int charge_pump_driver_init(void)
{
	...
	if (i2c_add_driver(&charge_pump_i2c_driver) != 0) {
		chg_err(" failed to register charge_pump_i2c_driver i2c driver.\n");
	} else {
		chg_debug( " Success to register charge_pump_i2c_driver i2c driver.\n");
	}

	return ret;
}
EXPORT_SYMBOL(charge_pump_driver_init);

void charge_pump_driver_exit(void)
{
	i2c_del_driver(&charge_pump_i2c_driver);
}
EXPORT_SYMBOL(charge_pump_driver_exit);
