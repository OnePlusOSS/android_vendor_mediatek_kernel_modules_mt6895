...
int oplus_usbtemp_dischg_action(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		if (oplus_usbtemp_check_is_platpmic(chip) == true) {
			if (chip->chg_ops->set_dischg_enable != NULL) {
				chg_err("platpmic set_dischg_enable\n");
				chip->chg_ops->set_dischg_enable(true);
			} else {
				chg_err("platpmic set_dischg_enable is NULL, the charger is not disable.\n");
			}
		} else {
...
static void usbtemp_restart_work(struct work_struct *work)
{
	...
	if (level == 1) {
		...
		if (oplus_usbtemp_check_is_platpmic(chip) == true) {
                        if (chip->chg_ops->set_dischg_enable != NULL) {
                                chg_err("platpmic set_dischg_enable false\n");
                                chip->chg_ops->set_dischg_enable(false);
                        } else {
                                chg_err("platpmic set_dischg_enable is NULL, the charger is not enable.\n");
                        }
                } else {
			pinctrl_select_state(chip->normalchg_gpio.pinctrl, chip->normalchg_gpio.dischg_disable);
		}
...
static void oplus_usbtemp_recover_tbatt_temp(struct oplus_chg_chip *chip) {
	...
	} else if (oplus_gauge_is_fuelgauge_apply()) {
		/*use battery protect board  NTC*/
		g_tbatt_temp = oplus_gauge_get_batt_protect_board_temperature();
	} else {
