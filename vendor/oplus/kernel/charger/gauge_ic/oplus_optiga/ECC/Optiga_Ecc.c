...
BOOL Ecc_DoAuthenticationEnhanced(uint8_t *gf2nUid )
{
...
#endif
		{
			printk(" Ecc_DoAuthenticationEnhanced  Ecc_ReadODC failed.\n");
			return FALSE;
		}
	} 
	...
	printk(" Ecc: Read ODC %04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
		gf2nODC_first[0],
		gf2nODC_first[1],
		gf2nODC_first[2],
		gf2nODC_first[3],
	...
	printk(" Ecc: Read ODC key: %04x:%04x:%04x:%04x:%04x:%04x\n",
		gf2nPublicKey_first[0],
		gf2nPublicKey_first[1],
		gf2nPublicKey_first[2],
		gf2nPublicKey_first[3],
		gf2nPublicKey_first[4],
		gf2nPublicKey_first[5]);

	printk(" Ecc_DoAuthenticationEnhanced: Ecc_VerifyODC\n");
	...
	printk(" Ecc_DoAuthenticationEnhanced: Ecc_GenerateChallenge\n");
	if (Ecc_GenerateChallenge(gf2nChallenge, gf2nRandomValue, MODE_ECCE ) == FALSE)
	{
		printk(" Ecc_DoAuthenticationEnhanced Ecc_GenerateChallenge failed.\n");
		return FALSE;
	}

	printk(" Ecc_DoAuthenticationEnhanced: Ecc_GenerateCheckValue\n");
	if (Ecc_GenerateCheckValue(gf2nCheck, gf2nRandomValue, gf2nPublicKey, MODE_ECCE )== FALSE)
	{
		printk(" Ecc_DoAuthenticationEnhanced Ecc_GenerateCheckValue failed.\n");
		return FALSE;
	}
	...
	{
		printk(" Ecc_DoAuthenticationEnhanced  Ecc_StartECC failed.\n");
		return FALSE;
	}

	printk(" Ecc_DoAuthenticationEnhanced: Ecc_VerifyResponse\n");
	if (Ecc_VerifyResponse(gf2nReturnX, gf2nReturnZ, gf2nCheck, gf2nPublicKey, MODE_ECCE ) == FALSE )
	{
		printk(" Ecc_DoAuthenticationEnhanced Ecc_VerifyResponse failed.\n");
		return FALSE;
	}

	readdoc_done = TRUE;
	printk(" Ecc_DoAuthenticationEnhanced: readdoc_done=%s\n", readdoc_done == TRUE?"true":"false");
	return TRUE;
}
#endif
...
BOOL Ecc_StartECC(dwordvec_t gf2n_Challenge, dwordvec_t gf2n_XResponse, dwordvec_t gf2n_ZResponse, BOOL bPolling, uint8_t bEccMode)
{
	...
	ret = Ecc_SendChallengeAndGetResponse((uint16_t *)gf2n_Challenge, (uint32_t *)gf2n_XResponse, (uint32_t *)gf2n_ZResponse, bPolling, bEccMode);
	if (ret == FALSE)
		return ret;
#if defined (SLE95150) || defined (SLE95250) || defined (SLE95300)
	if (bEccMode == MODE_ECCE)
