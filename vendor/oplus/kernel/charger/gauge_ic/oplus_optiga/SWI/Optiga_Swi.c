...
extern uint32_t g_ulBaudLow;				//1 * TAU
extern uint32_t g_ulBaudHigh;				//3 * TAU
extern uint32_t g_ulBaudStop;				//5 * TAU
extern uint32_t g_ulResponseTimeOut;		//10 * TAU
extern uint32_t g_ulResponseTimeOutLong;	//30ms
extern uint32_t g_ulBaudPowerDownTime;		//196us
extern uint32_t g_ulBaudPowerUpTime;		//10ms
extern uint32_t g_ulBaudResetTime;			//1ms
extern uint32_t g_tau_base;
...
#define OPTIA_DBUG_PRINT (0)
extern BOOL test_read;

BOOL Swi_ReceiveRawWord( uint8_t * up_Byte )
{
	BOOL ret = TRUE;
	volatile BOOL bPreviousSwiState = TRUE;
	uint8_t ubIndex = 12u;
	...
	if (ulTimeOut == 0u)
	{
		printk("Ecc Swi_ReceiveRawWord 01");
		return FALSE;
	}
	...
	for( ubBitsToCapture = 13u; ubBitsToCapture != 0u; ubBitsToCapture-- )
	{
		...
		while (get_pin() == bPreviousSwiState && ulTimeOut )
		{
			ulCount++;
			ulTimeOut--;
			ic_udelay(g_tau_base);
		}
		ulTimes[ubIndex] = ulCount;
		uTimes_temp[ubIndex]= ulCount;
		ubIndex--;
		bPreviousSwiState = !bPreviousSwiState;
	}
...
#if OPTIA_DBUG_PRINT
	if (ret == FALSE) {
		printk("Ecc read failed\n");
	} else {
		printk("Ecc read success\n");
	}
#endif

	if (test_read) {
		printk("Ecc g_tau_base:%d,ulThreshold:%04x, ulMaxTime:%04x, ulMinTime:%04x, uTimes_temp:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x\n",
			g_tau_base,
			ulThreshold,
                        ulMaxTime,
                        ulMinTime,
