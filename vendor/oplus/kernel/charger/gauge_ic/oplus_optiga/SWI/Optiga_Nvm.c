...
/*
output:     uint8_t *ubp_Data
return:    'TRUE', if ok.
		   'FALSE', if  failed.

*/
BOOL Nvm_ReadODC_Page(struct oplus_optiga_chip * optiga_chip_info,
						uint16_t uw_Address,
						uint8_t *ubp_Data,
						uint8_t page)
{
	...
	Swi_WriteRegisterSpaceNoIrq(CTRL2_NVM_REG, (uint8_t)(0x97 + page), REGISTER_OVERWRITE);
	ret = Nvm_IsReady();
	if (ret != APP_NVM_READY) {
		spin_unlock_irqrestore(&optiga_chip_info->slock, flags);
		printk("Nvm_ReadODC_Page  =Nvm_IsReady failed page=%d \n", page);
		return FALSE;
	}
	spin_unlock_irqrestore(&optiga_chip_info->slock, flags);

#ifdef BURST_READ_INTERVAL
	mdelay(DELAY_5MS); /*delay to schedule the task*/
#endif

	ret = Swi_ReadRegisterSpaceBurst(NVM_WIP2_REG, ubp_Data);
	if (ret != TRUE) {
		printk("Nvm_ReadODC Swi_ReadWIPRegisterBurst page=%d failed\n", page);
		return FALSE;
	}
...
uint16_t Nvm_ReadODC(uint8_t *ubODC, uint8_t *ubPUBKEY, NVM_OPS ReadMode)
{
...
#ifdef BURST_READ_INTERVAL
	mdelay(DELAY_20MS); /*20ms to schedule the task*/
#endif
	...
	for (j = 0; j < (NVM_ODC_PAGE_COUNT + NVM_PUK_PAGE_COUNT); j++) { /*!< Read 9 pages from NVM, starting from page 23 */
		...
		for (i = 0; i < READ_RETRY; i++) {
			if (TRUE == Nvm_ReadODC_Page(optiga_chip_info, NVM_WIP2_REG, _ubData + (NVM_PAGE_SIZE * j), j)) {
				break;
			} else {
				printk(" Nvm_ReadODC end j =%d, retry = %d failed.\n", j, i);
			}

#ifdef BURST_READ_INTERVAL
			mdelay(DELAY_5MS); /*delay schedule the task*/
#endif
		}

		if (i >= READ_RETRY) {
			printk(" Nvm_ReadODC end j=%d, retry=%d timeout.\n", j, i);
			return INF_SWI_E_TIMEOUT;
		}
#ifdef BURST_READ_INTERVAL
		mdelay(DELAY_5MS); /*delay to schedule the task*/
#endif
		printk(" Nvm_ReadODC end j=%d, retry=%d\n", j, i);
	}
