...
#define OPLUS_OPTIGA_DELAY_MSECS    (3000)
#define OPLUS_OPTIGA_CPU            (7) /*Default is 7.*/
...
S_OPTIGA_PUID g_optiga_uid;
static bool oplus_optiga_read_uid(int speed)
{
	unsigned long flags;

	if (NULL == g_oplus_optiga_chip) {
		return false;
	}
...
#ifdef BURST_READ
	if (Swi_ReadUID_burst((uint8_t *)&g_optiga_uid, HIGH_BYTE_FIRST, BURST_BYTES_READ) != INF_SWI_SUCCESS)
#else
	if (Swi_ReadUID((uint8_t *)&g_optiga_uid) == FALSE)
#endif
	{
		spin_lock_irqsave(&g_oplus_optiga_chip->slock, flags);
		Swi_PowerDownCmd();
		spin_unlock_irqrestore(&g_oplus_optiga_chip->slock, flags);
		test_read = FALSE;
		chg_err("oplus_optoga_read_uid:Select By Address Failed\n");
		return false;
	}
	test_read = FALSE;
	chg_err("optiga read: uid:0x%x, vid:0x%x, ulIdHigh:0x%x, ulIdLow:0x%x\n",
			g_optiga_uid.uwProductId,
			g_optiga_uid.uwVendorId,
			g_optiga_uid.ulIdHigh,
			g_optiga_uid.ulIdLow);
	return true;
}
...
int optiga_authenticate(void)
{
	...
	testNum++;
	chg_err("optiga_authenticate devloop:%d, testNum=%d\n", devloop, testNum);

	/*Note: the speed shall be update when change to new platform.*/
	timing_init(2);
	Swi_PowerDown();
	Swi_PowerUp();
	//Swi_Reset();
	...
	//Swi_SetAddress(1);
	chg_err("ECCE uwProductId:0x%x, uwVendorId:0x%x, ulIdHigh:0x%x, ulIdLow:0x%x\n",
			stDetectedPuids[0].uwProductId,
			stDetectedPuids[0].uwVendorId,
			stDetectedPuids[0].ulIdHigh,
			stDetectedPuids[0].ulIdLow);
...
BOOL test_gpio_write_time(void)
{
        int ubIndex = 0;
        unsigned long flags;
        ...
        printk("test_gpio_write_time start.. \n");

        spin_lock_irqsave(&optiga_chip_info->slock, flags);
        for (ubIndex = 1000; ubIndex != 0u; ubIndex--)
        {
                set_pin(0);
                set_pin(1);
        }
        spin_unlock_irqrestore(&optiga_chip_info->slock, flags);
        printk("test_gpio_write_time end. \n");
        return TRUE;
}
...
int oplus_optiga_start_test(int count) {
	...
	if (count == 101) {
		oplus_optiga_read_uid(1);
	} else if (count == 102) {
                oplus_optiga_read_uid(2);
	} else if (count == 111) {
		test_gpio_write_time();
	} else {
		...
		schedule_delayed_work_on(OPLUS_OPTIGA_CPU, &g_oplus_optiga_chip->test_work, 0);
	}
	return 0;
}
...
static void oplus_optiga_auth_work(struct work_struct *work)
{
	...
	if (ret == false) {
		...
		if (try_count < g_oplus_optiga_chip->try_count) {
			chg_err("optiga_authenticate try_count = %d!\n", try_count);
			schedule_delayed_work_on(OPLUS_OPTIGA_CPU, &g_oplus_optiga_chip->auth_work, 0);
			return;
		} else {
			chg_err("optiga_authenticate try_count = %d failed.!\n", try_count);

			/*set the authenticate_result as false after try_count.*/
		...
		chg_err("optiga_authenticate try_count = %d success!\n", try_count);
		complete(&g_oplus_optiga_chip->is_complete);
	}
...
int oplus_optiga_auth(int retry_count)
{
	if (!g_oplus_optiga_chip) {
		return 0;
	}
	...
	if (!wait_for_completion_timeout(&g_oplus_optiga_chip->is_complete,
			msecs_to_jiffies(OPLUS_OPTIGA_DELAY_MSECS * retry_count))) {
		chg_err("time out, authenticate failed.!\n");
	}
...
static void oplus_optiga_test_func(struct work_struct *work)
{
	...
	while (g_oplus_optiga_chip->test_result.test_count_now < g_oplus_optiga_chip->test_result.test_count_total) {
		...
		/*clear the authenticate_result status*/
                g_oplus_optiga_chip->hmac_status.authenticate_result = false;
		ret = oplus_optiga_auth(TEST_COUNT);
		if (ret == false) {
...
int oplus_optiga_get_external_auth_hmac(void) {
	...
	if (g_oplus_optiga_chip->hmac_status.authenticate_result == false) {
		...
		g_oplus_optiga_chip->try_count = OPLUS_OPTIGA_RETRY_COUNT;
		ret = oplus_optiga_auth(OPLUS_OPTIGA_RETRY_COUNT);
		if (ret == false) {
...
static int oplus_optiga_probe(struct platform_device *pdev)
{
	...
	/*add for test the GPIO write speed and stablity.*/
	// test_gpio_write_time();

	if (oplus_optia_check_auth_msg()) {
