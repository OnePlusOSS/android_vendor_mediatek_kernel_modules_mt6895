...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	chip->usbtemp_dischg_by_platpmic = of_property_read_bool(node, "qcom,usbtemp_dischg_by_platpmic");
	charger_xlog_printk(CHG_LOG_CRTI, "usbtemp_dischg_by_platpmic=%d\n", chip->usbtemp_dischg_by_platpmic);
	return 0;
}
