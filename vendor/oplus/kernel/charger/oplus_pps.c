...
chg_exception:
	...
	pps_err("pps data[18~1C][%d %d %d %d %d], iic[%d, %d]\n",
...
static int oplus_pps_get_pps_status(struct oplus_pps_chip *chip)
{
	...
	pps_err("get volt:%d,cur:%d,pps_status:0x%x\n", volt, cur, pps_status);
...
static int oplus_pps_choose_curves(struct oplus_pps_chip *chip)
{
	...
	pps_err("pps_adapter_type: %d, temperature: %d, soc: %d, batt_soc_plugin: %d, batt_temp_plugin: %d",
...
static void oplus_pps_check_low_curr_temp_status(struct oplus_pps_chip *chip)
{
	...
	pps_err("vbat_temp_cur = %d, pps_low_curr_full_temp_status = %d, t_cnts = %d\n", vbat_temp_cur, chip->pps_low_curr_full_temp_status, t_cnts);
}
...
static int oplus_pps_check_ibus_curr(struct oplus_pps_chip *chip)
{
	...
	if ((chip->cp_slave_enable)
		&&((abs(chip->cp_master_ibus - chip->cp_slave_ibus) > 1000)
		|| (chip->cp_master_ibus > 4500)
		|| (chip->cp_slave_ibus > 4500))) {
...
static int oplus_pps_get_target_current(struct oplus_pps_chip *chip)
{
	...
	pps_err("current_batt_curve= %d, current_batt_temp= %d, current_cool_down= %d, cp_ibus_down= %d, cp_r_down= %d\n",
...
static int oplus_pps_action_check(struct oplus_pps_chip *chip)
{
	...
	pps_err("pps_status = %d, target_volt: %d, target_current = %d, ap_input_volt: %d, ask_charger_volt: %d, ask_charger_current: %d, ap_batt_current: %d\n",
...
static int oplus_pps_psy_changed(struct oplus_pps_chip *chip)
{
	if(chip->pps_batt_psy) {
		pps_err("oplus_pps_psy_changed pps_fastchg_started = %d\n", chip->pps_fastchg_started);
...
int oplus_pps_get_ffc_started(void) {
	...
	pps_err("oplus_pps_get_ffc_started pps_status = %d\n", chip->pps_status);
