...
#define BATTERY_DEFAYLT_TEMPERATURE (250)
#define BATTERY_MAX_TEMPERATURE     (690)
...
int oplus_gauge_get_batt_protect_board_temperature(void)
{
        int batt_temp = 0;

        if ((NULL == g_gauge_chip)
		|| (NULL == g_gauge_chip->gauge_ops)
		|| (NULL == g_gauge_chip->gauge_ops->get_battery_protect_board_temperature)) {
                return BATTERY_DEFAYLT_TEMPERATURE;
        } else {
                if (gauge_dbg_tbat != 0 || sub_gauge_dbg_tbat != 0) {
                        printk(KERN_ERR "[OPLUS_CHG]debug enabled, gauge_dbg_tbat[%d],  sub_gauge_dbg_ibat[%d]\n",
                                        gauge_dbg_tbat, sub_gauge_dbg_tbat);
                ...
                batt_temp = g_gauge_chip->gauge_ops->get_battery_protect_board_temperature();
		if (get_eng_version() == HIGH_TEMP_AGING) {
                        printk(KERN_ERR "[OPLUS_CHG]CONFIG_HIGH_TEMP_VERSION enable here, \
                                        disable high tbat shutdown \n");
                        if (batt_temp > BATTERY_MAX_TEMPERATURE)
                                batt_temp = BATTERY_MAX_TEMPERATURE;
                }
                return batt_temp;
        }
}

bool oplus_gauge_is_fuelgauge_apply(void)
{
	if ((NULL == g_gauge_chip)
                || (NULL == g_gauge_chip->gauge_ops)
                || (NULL == g_gauge_chip->gauge_ops->is_fuelgauge_apply)) {
                return false;
        } else {
		return g_gauge_chip->gauge_ops->is_fuelgauge_apply();
	}
}
