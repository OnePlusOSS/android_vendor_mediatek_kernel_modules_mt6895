...
#ifdef CONFIG_OPLUS_FEATURE_GALLERY3D_UX
	if (task->group_leader && strstr(task->group_leader->comm, "loros.gallery3d")) {
		if (strstr(buf, "ExoPlayerImplIn") || strstr(buf, "MediaCodec_loop"))
			oplus_set_ux_state(task, SA_TYPE_HEAVY);
	}
#endif
...
static inline bool im_mali(struct task_struct *p)
{
	return strstr(p->comm, "mali-event-hand");
}
static inline bool cgroup_check_set_sched_assist_boost(struct task_struct *p)
{
	return im_mali(p);
}

static void cgroup_set_sched_assist_boost_task(struct task_struct *p)
{
	struct rq_flags flags;
	struct rq *rq;
	int ux_state;
