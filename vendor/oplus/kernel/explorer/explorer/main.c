...
static int explorer_parse_clocks(struct explorer_plat_data *plat_priv)
{
	...
	if (of_property_read_u32(np, "explorer,ref-clk-idx", &clk_ref) != 0) {
		pr_err("%s, failed to parse explorer ref-clk-idx, use default!", __func__);
	}
	if (of_property_read_string(np, "explorer,ref-clk-voter", &clk_voter) != 0) {
		pr_err("%s, failed to parse explorer ref-clk-voter, use default!", __func__);
	}
	if (of_property_read_u32(np, "explorer,ref-clk-voter-id", &clk_voter_id) != 0) {
		pr_err("%s, failed to parse explorer ref-clk-voter-id, use default!", __func__);
	}
	pr_info("%s, parse clk_ref:%d clk_voter:%s clk_voter_id:%d.",
			__func__, clk_ref, clk_voter, clk_voter_id);

	plat_priv->clk_ref = clk_buf_get_xo_name(clk_ref);
	clk_buf_set_voter_by_name(plat_priv->clk_ref, clk_voter);
	plat_priv->clk_voter_id = clk_voter_id;
#endif
