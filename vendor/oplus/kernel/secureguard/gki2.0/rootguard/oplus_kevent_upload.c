...
static void oplus_kevent_send_to_user(void *data,
				      struct kernel_packet_info *userinfo, int *p_retval);

static int security_keventupload_sendpid_cmd(struct sk_buff *skb,
...
static int security_keventupload_test_upload(struct sk_buff *skb,
		struct genl_info *info)
{
	int ret = 0;
	struct nlattr *na = NULL;
				      ...
				      struct kernel_packet_info *userinfo, int *p_retval)
{
	...
	*p_retval = 0;
	...
	ret = genl_msg_prepare_usr_msg(SECURE_GUARD_CMD_GENL_UPLOAD, attr_len,
				       kevent_pid, &skbuff);
	...
	ret = genl_msg_mk_usr_msg(skbuff, SECURE_GUARD_CMD_ATTR_MSG, userinfo,
				  data_len);
