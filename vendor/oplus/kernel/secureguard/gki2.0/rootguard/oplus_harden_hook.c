...
static void oplus_secure_harden_kevent(uid_t new_id, uid_t new_eid,
				       uid_t new_sid, int flag, unsigned char uid_or_gid)
{
	int ret = -1;
	...
	strncpy(dcs_event->log_tag, OPLUS_HARDEN_CAPABILITY_EVENT_NAME,
		sizeof(dcs_event->log_tag));
	strncpy(dcs_event->event_id, OPLUS_HARDEN_CAPABILITY_EVENT_ID,
	...
	if ((uid_or_gid >= OPLUS_HARDEN_UID) && (uid_or_gid <= OPLUS_HARDEN_RESUID)) {
		dcs_event->payload_length = snprintf(dcs_event_payload, 256,
						     ...
						     get_fs(), current_uid().val, current_euid().val, current_suid().val,
						     get_task_comm(comm, current), selinux_enabled);

	} else if ((uid_or_gid >= OPLUS_HARDEN_GID)
			&& (uid_or_gid <= OPLUS_HARDEN_RESGID)) {
		dcs_event->payload_length = snprintf(dcs_event_payload, 256,
						     "%d$$new_egid@@%d$$new_sgid@@%d$$set_id_flag@@%d$$addr_limit@@%lx$$curr_gid@@%d$$curr_egid@@%d$$curr_sgid@@%d$$curr_name@@%s$$enforce@@%d\n",
						     new_id, new_eid, new_sid, flag,
						     get_fs(), current_gid().val, current_egid().val, current_sgid().val,
						     get_task_comm(comm, current), selinux_enabled);
		...
		if (cur_uid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			...
			if (new_uid == OPLUS_ANDROID_ROOT_UID) {
				pr_err("[ROOTCHECK-CAP-ERROR]:CAP security incident detected, old->uid.val is %u, new->uid.val is %u\n",
				       cur_uid, new_uid);
				oplus_secure_harden_kevent(new_uid, new_uid, new_uid, OPLUS_SETID_ID,
							   OPLUS_HARDEN_UID);
				/*restore the uid*/
				//regs->regs[0] = cur_uid;
			}
...
static int oplus_harden_setreuid(struct pt_regs *regs)
{
	...
	if (capable(CAP_SETUID) == true) {
		if (cur_uid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			if ((new_uid == OPLUS_ANDROID_ROOT_UID)
					|| (new_euid == OPLUS_ANDROID_ROOT_UID)) {
				...
				oplus_secure_harden_kevent(new_uid, new_euid, new_uid, OPLUS_SETID_RE,
							   OPLUS_HARDEN_EUID);
				/*restore the uid*/
				//regs->regs[0] = cur_uid;
	...
	if (capable(CAP_SETUID) == true) {
		if (cur_uid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			if ((new_uid == OPLUS_ANDROID_ROOT_UID) || (new_euid == OPLUS_ANDROID_ROOT_UID)
					|| (new_suid == OPLUS_ANDROID_ROOT_UID)) {
				...
				oplus_secure_harden_kevent(new_uid, new_euid, new_suid, OPLUS_SETID_RES,
							   OPLUS_HARDEN_RESUID);
				/*restore the uid*/
				//regs->regs[0] = cur_uid;
				//regs->regs[1] = cur_euid;
		...
		if (cur_gid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			if (new_gid == OPLUS_ANDROID_ROOT_UID) {
				pr_err("[ROOTCHECK-CAP-ERROR]:CAP security incident detected, old->gid.val is %u, new->gid.val is %u\n",
				       cur_gid, new_gid);
				oplus_secure_harden_kevent(new_gid, new_gid, new_gid, OPLUS_SETID_ID,
							   OPLUS_HARDEN_GID);
				/*restore the gid*/
				//regs->regs[0] = cur_gid;
			}
...
static int oplus_harden_setregid(struct pt_regs *regs)
{
	...
	if (capable(CAP_SETGID) == true) {
		if (cur_gid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			if ((new_gid == OPLUS_ANDROID_ROOT_UID)
					|| (new_egid == OPLUS_ANDROID_ROOT_UID)) {
				...
				oplus_secure_harden_kevent(new_gid, new_egid, new_gid, OPLUS_SETID_RE,
							   OPLUS_HARDEN_EGID);
				/*restore the gid*/
				//regs->regs[0] = cur_gid;
	...
	if (capable(CAP_SETGID) == true) {
		if (cur_gid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
			if ((new_gid == OPLUS_ANDROID_ROOT_UID) || (new_egid == OPLUS_ANDROID_ROOT_UID)
					|| (new_sgid == OPLUS_ANDROID_ROOT_UID)) {
				...
				oplus_secure_harden_kevent(new_gid, new_egid, new_sgid, OPLUS_SETID_RES,
							   OPLUS_HARDEN_RESGID);
				/*restore the gid*/
				//regs->regs[0] = cur_gid;
				//regs->regs[1] = cur_egid;
...
static int oplus_harden_socket_create(struct pt_regs *regs)
{
	...
	if (cur_uid >= OPLUS_ANDROID_THIRD_PART_APK_UID) {
		if ((family != AF_NETLINK) && (type == SOCK_RAW) && !capable(CAP_NET_RAW)) {
			oplus_secure_harden_kevent((uid_t)family, (uid_t)type, (uid_t)protocol,
						   OPLUS_HARDEN_NET_RAW_FLAG, OPLUS_HARDEN_NET_RAW);
