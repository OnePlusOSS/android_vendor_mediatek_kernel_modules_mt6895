...
void report_secuiry_event(const char *event_name, unsigned int event_type,
			  const char *more)
{
	...
	dcs_event->payload_length = snprintf(dcs_event_payload, 256,
					     "$$uid@@%d$$EVENT_TYPE@@%d$$current_name@@%s$$additional@@%s\n",
...
/* ---CONFIG_OPLUS_HP_CHECK --- */
static int entry_handler_socket(struct kretprobe_instance *ri,
				struct pt_regs *regs)
{
...
static int entry_handler_socket_ip6(struct kretprobe_instance *ri,
				    struct pt_regs *regs)
{
	int socket_type = regs->regs[1];
...
static int entry_handler_cpuinfo(struct kretprobe_instance *ri,
				 struct pt_regs *regs)
{
	oplus_heapspray_check(CPU_INFO);
	return 0;
}
static int entry_handler_setxattr(struct kretprobe_instance *ri,
				  struct pt_regs *regs)
{
...
/* ---CONFIG_OPLUS_SEPOLICY_RELOAD --- */
static int entry_handler_sepolicy_reload(struct kretprobe_instance *ri,
		struct pt_regs *regs)
{
	oplus_sepolicy_reload();
	return 0;
...
MODULE_PARM_DESC(func,
		 "This is a module developed by OPLUS's AMBER LAB to use a strengthened kernel to enhance the security capabilities of the kernel.");
