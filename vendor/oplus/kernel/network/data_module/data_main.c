...
extern void game_main_stream_monitor_exit(void);
...
static int __init data_modules_init(void)
{
	...
	ret = game_main_stream_monitor_init();
	if(ret){
	    goto game_main_stream_monitor_failed;
	}
	printk("data_modules_init succ \n");
	return 0;

game_main_stream_monitor_failed:
    game_main_stream_monitor_exit();
log_failed:
    wzry_stats_fini();
wzry_failed:
	tmgp_sgame_fini();
